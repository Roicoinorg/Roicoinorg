...
public:
    ...
    bool WriteZROISeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZROISeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZROISeed_deprecated(uint256& seed);
    bool EraseZROISeed();
    bool EraseZROISeed_deprecated();

    bool WriteZROICount(const uint32_t& nCount);
    bool ReadZROICount(uint32_t& nCount);
