...
#include "ui_zroicontroldialog.h"
...
std::set<std::string> ZRoiControlDialog::setSelectedMints;
std::set<CMintMeta> ZRoiControlDialog::setMints;

bool CZRoiControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRoiControlDialog::COLUMN_DENOMINATION || column == ZRoiControlDialog::COLUMN_VERSION || column == ZRoiControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRoiControlDialog::ZRoiControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRoiControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zROI Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZRoi->setProperty("cssClass", "text-main-purple");
    ui->labelZRoi_int->setProperty("cssClass", "text-main-purple");
...
ZRoiControlDialog::~ZRoiControlDialog()
{
    delete ui;
}

void ZRoiControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRoiControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRoiControlWidgetItem* itemDenom(new CZRoiControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRoiControlWidgetItem *itemMint = new CZRoiControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zroiTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zroiTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zROI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zROI seed used to mint this zROI is not the same as currently hold in the wallet";
            else
...
void ZRoiControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRoiControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRoi_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZRoiControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRoiControlDialog::GetSelectedMints()
{
...
void ZRoiControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
