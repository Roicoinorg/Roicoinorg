...
#include "qt/roicoin/privacywidget.h"
#include "qt/roicoin/forms/ui_privacywidget.h"
#include "qt/roicoin/qtutils.h"
#include "guiutil.h"
#include "qt/roicoin/denomgenerationdialog.h"
#include "qt/roicoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(ROICOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zROI anonymizes your ROI by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zROI or convert back to ROI"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 ROI ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zROI");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zROI");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zROI");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zROI");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zROI");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zROI");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zROI");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zROI");
    ...
    // Only Convert to ROI enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzROI->setTitleClassAndText("btn-title-grey", "Total 0 zROI");
    ui->btnTotalzROI->setSubTitleClassAndText("text-subtitle", "Show denominations of zROI owned.");
    ui->btnTotalzROI->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ROI outputs to mint into zROI.");
    ...
    connect(ui->btnTotalzROI, SIGNAL(clicked()), this, SLOT(onTotalZroiClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zROI");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of ROI to mint into zROI"));
    }else{
        btnText = tr("Convert back to ROI");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zROI to convert back into ROI"));
    }
...
void PrivacyWidget::onTotalZroiClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzROI->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzROI->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zROI is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zROI, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zROI").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zROI minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZroi(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zROI converted back to ROI"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zROI </b>";
    ...
    ui->btnTotalzROI->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
