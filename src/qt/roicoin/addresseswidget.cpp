...
#include "qt/roicoin/addresseswidget.h"
#include "qt/roicoin/forms/ui_addresseswidget.h"
#include "qt/roicoin/addresslabelrow.h"
#include "qt/roicoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(ROICOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a ROICOIN address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress roiAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(roiAdd.Get(), label.toUtf8().constData(),
                roiAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
