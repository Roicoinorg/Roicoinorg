...
#include "qt/roicoin/topbar.h"
#include "qt/roicoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/roicoin/lockunlock.h"
#include "qt/roicoin/qtutils.h"
#include "qt/roicoin/receivedialog.h"
...
TopBar::TopBar(ROICOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopRoi, ui->labelAmountTopzRoi}, "amount-small-topbar");
    setCssProperty({ui->labelAmountRoi, ui->labelAmountzRoi}, "amount-topbar");
    setCssProperty({ui->labelPendingRoi, ui->labelPendingzRoi, ui->labelImmatureRoi, ui->labelImmaturezRoi}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzRoi->setVisible(false);
    ui->labelAmountzRoi->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzRoi->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezRoi->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("ROICOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount roiAvailableBalance = balance + delegatedBalance;

    // zROI Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalRoi = GUIUtil::formatBalance(roiAvailableBalance, nDisplayUnit);
    QString totalzRoi = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopRoi->setText(totalRoi);
    ui->labelAmountTopzRoi->setText(totalzRoi);

    // Expanded
    ui->labelAmountRoi->setText(totalRoi);
    ui->labelAmountzRoi->setText(totalzRoi);

    ui->labelPendingRoi->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzRoi->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureRoi->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezRoi->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
