...
#include "qt/roicoin/send.h"
#include "qt/roicoin/forms/ui_send.h"
#include "qt/roicoin/addnewcontactdialog.h"
#include "qt/roicoin/qtutils.h"
#include "qt/roicoin/sendchangeaddressdialog.h"
#include "qt/roicoin/optionbutton.h"
#include "qt/roicoin/sendconfirmdialog.h"
#include "qt/roicoin/myaddressrow.h"
#include "qt/roicoin/guitransactionsutils.h"
...
#include "zroicontroldialog.h"

SendWidget::SendWidget(ROICOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("ROI");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zROI");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (ROI)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a ROICOIN address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 ROI");
    ...
    setCssProperty(coinIcon, "coin-icon-roi");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onROISelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onROISelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZroi = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZroi));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZroi ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendRoi = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendRoi ? AskPassphraseDialog::Context::Send_ROI : AskPassphraseDialog::Context::Send_zROI, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendRoi) ? send(recipients) : sendZroi(recipients)) {
...
bool SendWidget::sendZroi(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zROI is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zROI selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZRoiControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZRoiControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZroi(
        ...
        inform(tr("zROI transaction sent!"));
        ZRoiControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZROI_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zROI require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zROI transaction
        ...
        emit message("zROI transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isROI){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any ROI to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZRoiControlDialog *zRoiControl = new ZRoiControlDialog(this);
            zRoiControl->setModel(walletModel);
            zRoiControl->exec();
            ui->btnCoinControl->setActive(!ZRoiControlDialog::setSelectedMints.empty());
            zRoiControl->deleteLater();
        } else {
            inform(tr("You don't have any zROI in your balance to select."));
        }
    }
}
...
void SendWidget::onROISelected(bool _isROI){
    isROI = _isROI;
    setCssProperty(coinIcon, _isROI ? "coin-icon-roi" : "coin-icon-zroi");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress roiAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(roiAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(roiAdd.Get(), dialog->getLabel().toStdString(), "send")) {
