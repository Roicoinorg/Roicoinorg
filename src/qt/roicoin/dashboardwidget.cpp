...
#include "qt/roicoin/dashboardwidget.h"
#include "qt/roicoin/forms/ui_dashboardwidget.h"
#include "qt/roicoin/sendconfirmdialog.h"
#include "qt/roicoin/txrow.h"
...
DashboardWidget::DashboardWidget(ROICOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of ROI staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareRoi, "square-chart-roi");
    setCssProperty(ui->labelSquarezRoi, "square-chart-zroi");
    setCssProperty(ui->labelRoi, "text-chart-roi");
    setCssProperty(ui->labelZroi, "text-chart-zroi");
    ui->labelZroi->setVisible(false);
    ui->labelSquarezRoi->setVisible(false);
    ...
    ui->labelAmountZroi->setText("0 zROI");
    ui->labelAmountZroi->setVisible(false);
    ui->labelAmountRoi->setText("0 ROI");
    setCssProperty(ui->labelAmountRoi, "text-stake-roi-disable");
    setCssProperty(ui->labelAmountZroi, "text-stake-zroi-disable");
    ...
    ui->btnHowTo->setText(tr("How to get ROI"));
...
void DashboardWidget::changeChartColors(){
    ...
    if(isLightTheme()){
        ...
        gridLineColorX = QColor(20, 10, 11);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isRoi = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZROI;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair ROI, zROI
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            roi = (pair.first != 0) ? pair.first / 100000000 : 0;
            zroi = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalRoi += pair.first;
            chartData->totalZroi += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesRoi.append(roi);
        chartData->valueszRoi.append(zroi);

        int max = std::max(roi, zroi);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("ROI");
    set1 = new QBarSet("zROI");
    set0->setColor(QColor(241, 56, 69));
    ...
    set0->append(chartData->valuesRoi);
    set1->append(chartData->valueszRoi);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalRoi > 0 || chartData->totalZroi > 0) {
        setCssProperty(ui->labelAmountRoi, "text-stake-roi");
        setCssProperty(ui->labelAmountZroi, "text-stake-zroi");
    } else {
        setCssProperty(ui->labelAmountRoi, "text-stake-roi-disable");
        setCssProperty(ui->labelAmountZroi, "text-stake-zroi-disable");
    }
    forceUpdateStyle({ui->labelAmountRoi, ui->labelAmountZroi});
    ui->labelAmountRoi->setText(GUIUtil::formatBalance(chartData->totalRoi, nDisplayUnit));
    ui->labelAmountZroi->setText(GUIUtil::formatBalance(chartData->totalZroi, nDisplayUnit, true));
