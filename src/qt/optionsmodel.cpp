...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ROI);
    ...
    if (settings.contains("nAnonymizeROIcoinAmount") || reset)
        SoftSetArg("-anonymizeroicoinamount", settings.value("nAnonymizeROIcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in roicoin.cpp during shotdown to also remove the window positions
