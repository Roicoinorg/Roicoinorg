...
//
// Bootup the Masternode, look for a 10000 ROICOIN input and register on the network
//
...
std::vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == GetMasterNodeCollateral(chainActive.Height()) * COIN) { //exactly
