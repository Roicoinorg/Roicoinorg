...
/**
 * ROICOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZroiSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzroiseed(const UniValue& params, bool fHelp);
extern UniValue getzroiseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzroi(const UniValue& params, bool fHelp);
extern UniValue dzroistate(const UniValue& params, bool fHelp);
