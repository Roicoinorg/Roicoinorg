...
CZRoiStake::CZRoiStake(const libzerocoin::CoinSpend& spend)
{
...
int CZRoiStake::GetChecksumHeightFromMint()
{
...
int CZRoiStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZRoiStake::GetChecksum()
{
    return nChecksum;
}

// The zROI block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZRoiStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZRoiStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zROI staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZRoiStake::GetUniqueness()
{
    //The unique identifier for a zROI is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZRoiStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZRoiStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zROI that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZROIOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zROI output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zROI", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZROIOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zROI output", __func__);
...
bool CZRoiStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZRoiStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzROITracker* zroiTracker = pwallet->zroiTracker.get();
    CMintMeta meta;
    if (!zroiTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zroiTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!ROI Stake
bool CRoiStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CRoiStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CRoiStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CRoiStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CRoiStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CRoiStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CRoiStake::GetUniqueness()
{
    //The unique identifier for a ROI stake is the outpoint
...
CBlockIndex* CRoiStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
