...
CzROIWallet::CzROIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zroi seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZROISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZROISeed_deprecated()) {
                    LogPrintf("%s: Updated zROI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zroi seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zroi wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzROIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZROICount(nCountLastUsed);
    else if (!walletdb.ReadZROICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzROIWallet::Lock()
{
    seedMaster = 0;
}

void CzROIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzROIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZROI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzROIWallet::LoadMintPoolFromDB()
{
...
void CzROIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzROIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzROIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zroiTracker->HasPubcoinHash(pMint.first)) {
...
bool CzROIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZROI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zroiTracker which also adds to database
    pwalletMain->zroiTracker->Add(dMint, true);
...
void CzROIWallet::SeedToZROI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzROIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzROIWallet::GenerateDeterministicZROI(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzROIWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzROIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzROIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
