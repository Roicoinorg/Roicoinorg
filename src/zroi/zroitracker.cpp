// Copyright (c) 2018-2019 The ROICOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zroi/deterministicmint.h>
#include "zroitracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzROITracker::CzROITracker(std::string strWalletFile)
{
...
CzROITracker::~CzROITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzROITracker::Init()
{
...
bool CzROITracker::Archive(CMintMeta& meta)
{
...
bool CzROITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzROITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzROITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzROITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzROITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzROITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzROITracker::GetSerialHashes()
{
...
CAmount CzROITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzROITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzROITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzROITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzROITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzROITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzROITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzROITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzROITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzROITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzROITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzROIWallet* zROIWallet)
{
    bool iszROIWalletInitialized = (NULL != zROIWallet);
    CMintMeta meta;
    ...
    if (! iszROIWalletInitialized)
        zROIWallet = new CzROIWallet(strWalletFile);
    meta.isSeedCorrect = zROIWallet->CheckSeed(dMint);
    if (! iszROIWalletInitialized)
        delete zROIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzROITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzROITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzROITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzROITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzROITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzROITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzROIWallet* zROIWallet = new CzROIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zROIWallet);
        }
        delete zROIWallet;
        LogPrint("zero", "%s: added %d dzroi from DB\n", __func__, listDeterministicDB.size());
    }
