...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "roicoin" is a composite category enabling all ROICOIN-related debug output
            if (ptrCategory->count(std::string("roicoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ROICOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ROICOIN
// Mac: ~/Library/Application Support/ROICOIN
// Unix: ~/.roicoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ROICOIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "roicoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty roicoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override roicoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "roicoind.pid"));
