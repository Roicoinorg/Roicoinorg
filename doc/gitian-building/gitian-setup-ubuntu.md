...
The build script we'll be using is contained in the ROICOIN github repository ([contrib/gitian-build.py](https://github.com/ROIcoin/roicoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ROICOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/ROIcoin/ROICOIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, roicoin-detached-sigs, and roicoin GitHub repos.
