...
*Setup instructions for a gitian build of Roi Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ROICOIN
...
to the ROICOIN GitHub release page.
...
- [Building Roi Coin](#building-roicoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/roicoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/roicoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/roicoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/roicoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/roicoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/roicoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ROIcoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@roicoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/roicoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/roicoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/roicoin-osx-*-build.assert
```
